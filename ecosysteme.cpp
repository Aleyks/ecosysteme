...
public:
  Tableau(int lignes, int colonnes) : lignes(lignes),
                                      colonnes(colonnes)
  {
...
public:
  Animal(int vie, int faim, int positionX, int positionY, char type) : vie(vie),
                                                                       faim(faim),
                                                                       positionX(positionX),
                                                                       positionY(positionY)
  {
...
{
public:
  Herbe(int positionX, int positionY) : Animal(1, 0, positionX, positionY, type()) {}
  char type() const
      override
  {
    if (vie >= 1)
...
{
public:
  Mouton(int positionX, int positionY) : Animal(50, 45, positionX, positionY, type()) {}
  ...
  void deplacer() override
  {
    ...
    case 9:
      ...
      break;
    case 1:
      if (positionX < tableau.getlignes() - 1)
        positionX++;
      break;
    case 8:
  ...
  {
    if (other.type() == 'H' && est_proche(other))
    {
...
{
public:
  Loup(int positionX, int positionY) : Animal(60, 40, positionX, positionY, type()) {}
  char type() const override
  {
    return vie ? 'L' : ' ';
  }
...
int main()
{
  ...
  while (true)
  {
    ...
    if (moutons.empty() && loups.empty())
    {
      break;
    }

    std::cout << "Ecosysteme terminé après " << compteur_tour << " tours." << std::endl;
  }
  return 0;
}