...
/* struct Case {
  int sol; // type de sol {herbe,vide,sel}
  Animal* a; // animal sur la case.
} */
...
int main() {
  ...
  std::vector<Mouton> moutons = {Mouton(0, 0), Mouton(1, 1),   Mouton(7, 1),
                                 Mouton(9, 9), Mouton(11, 11), Mouton(10, 10),
                                 Mouton(8, 8), Mouton(6, 6),   Mouton(4, 4),
                                 Mouton(3, 3), Mouton(2, 2)};

  std::vector<Loup> loups = {Loup(4, 5), Loup(2, 6)};

  int compteur_tour = 0;
  tab.afficher();
  while (true) {
    for (auto &mouton : moutons) {
      int prochain_X = mouton.getX();
      int prochain_Y = mouton.getY();
      //  mouton.deplacer();
      for (auto &herbe : herbes) {
        if (herbe.getX() == prochain_X && herbe.getY() == prochain_Y) {
          mouton.manger(herbe);
        }
        mouton.deplacer();
